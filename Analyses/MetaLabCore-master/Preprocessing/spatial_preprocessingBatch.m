...
% Ouden's code
% Adapted by Nadine Dijkstra in 2021 to work with MB4 data without fieldmap
% correction
...
if coregister
    ...
    for s0 = 1 : length(name_subj)
        ...
        f  = spm_select('List', sourceDir, '^meanuf.*\.nii$');
...
if normalise
    ...
    for s0 = 1 : length(name_subj)
        ...
        for sess = 1:n_sess
            scanDir = [dir_base fs name_subj{s0} fs dir_epi fs sess_prfx num2str(sess)];
            % select unwarped
            f   = spm_select('List', scanDir, '^uf.*\.nii$');
            files  = cellstr([repmat([scanDir fs],size(f,1),1) f]);
...
if smoothing
    ...
    for s0 = 1 : length(name_subj)
        ...
        for sess = 1:n_sess
            scanDir = [dir_base fs name_subj{s0} fs dir_epi fs sess_prfx num2str(sess)];
            f   = spm_select('List', scanDir, '^wuf.*\.nii$');
            files  = cellstr([repmat([scanDir fs],size(f,1),1) f]);
            conCat_files = [conCat_files; files];       % concatenate all files over runs
            f = []; files = [];
        end
