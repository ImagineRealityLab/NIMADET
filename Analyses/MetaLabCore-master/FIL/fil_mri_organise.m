function fil_mri_organise(cfg,subj)
...
for i_s = cfg.which_subjects
    old_root = fullfile(cfg.dir_root, subj{i_s}.name,'MRI','raw', subj{i_s}.scanid);
    new_root = fullfile(cfg.dir_root, subj{i_s}.name);

    for scan = cfg.which_scans
        switch scan{:}
            case 'loc'
                old_path = [old_root, num2str(subj{i_s}.localiser)];
                new_path = fullfile(new_root, dir_loc);
                pattern = '^s.*\.nii$';
                reorganise(old_path, new_path, pattern, remove_old);
            case 'struc'
                old_path = [old_root, num2str(subj{i_s}.structural)];
