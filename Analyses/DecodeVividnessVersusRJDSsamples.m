function DecodeVividnessVersusRJDSsamples(cfg)
% function DecodeVividnessVersusRJDSsamples(cfg)
%
...
%
% Downsample ds times to reduce influence randomness of downsampling

rng(1); % for reproduceability

nDS = 25; % random downsample iterations 
...
acc   = nan(nSub,2,nROIs,nDS); % vividness and RJ
pAcc  = nan(nSub,2,nROIs,cfg.nPerm,nDS);
    ...
    if ~isempty(X)
        ...
        for r = 1:nRuns
            ...
            if cfg.balancetrialsRJViv
                X(Ridx==run,:) = X(Ridx==run,:)-nanmean(X(Ridx==run,:),2); % average over voxels
            else
        ...
        for r = 1:nROIs
            ...
            if cfg.balancetrialsRJViv
                ...
                % calculate accuracy
                acc(sub,1,r,ds) = mean((yhatV>0)==Y1); pAcc(sub,1,r,:,ds) = mean((yhatPV>0)==repmat(Y1,1,cfg.nPerm));
                acc(sub,2,r,ds) = mean((yhatR>0)==Y2); pAcc(sub,2,r,:,ds) = mean((yhatPR>0)==repmat(Y2,1,cfg.nPerm));
                ...
                for type = 1:2
                    ...
                    % calculate accuracy
                    acc(sub,type,r,ds) = mean((yhat>0)==y); pAcc(sub,type,r,:,ds) = mean((yhatP>0)==repmat(y,1,cfg.nPerm));
                end
            end
        end
...
save(fullfile(group_dir,'ROI_decodingNotBalancedRJVivRandomDownsamples'),'acc','pAcc');


% remove nans
nan_idx = squeeze(any(any(isnan(squeeze(mean(acc,4))),2),3));
...
for r = 1:nROIs
    ...
    tmp = squeeze(acc(:,:,r,:));
    M   = squeeze(mean(tmp));
    SEM = squeeze(std(tmp))./sqrt(nSub);
    barwitherr(SEM',M'); title(cfg.ROIs{r});
    ylim([0.4 0.6]); hold on; plot(xlim,[0.5 0.5],'k--')
end

% average over ds's
acc = squeeze(mean(acc,4)); pAcc = squeeze(mean(pAcc,5));

figure;
b = barwitherr(squeeze(std(acc))'./sqrt(nSub),squeeze(mean(acc))');
hold on; plot(xlim,[0 0],'k--'); title('Accuracy');
