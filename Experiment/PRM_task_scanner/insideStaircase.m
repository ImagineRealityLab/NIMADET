...
%% inside staircase detection (SD) script
% Opens window -> defines trials -> makes gabors -> adds noise ->
% runs through trials.

function [V,acc] = insideStaircase(subID,orientation,gratingName,V,environment)
...
% show instructions
text = ['We will now do a calibration on the gratings in noise while we do some scans. \n ',...
...
for iTrial = 1:nTrials
    ...
    if stairCount==nEvalTrials
        ...
        if (acc(stairStep) > upper_bound) || (acc(stairStep) < lower_bound)
            visibility = visibility - round((acc(stairStep)-threshold)*90);
...
Screen('TextSize',w, 28);
DrawFormattedText(w, 'This is the end of the first task!', 'center', 'center', [255 255 255]);
vbl = Screen('Flip', w);
WaitSecs(2);
Screen('CloseAll')
sca; 